"""
Copy from tools/dataset_converters/yolo2coco.py, modified for MMPTrack dataset conversion.
"""
...
def convert_bbox_info(label, idx, obj_count, image_height, image_width):
    ...
    x1 = max(0, min(float(label[1]), image_width))
    y1 = max(0, min(float(label[2]), image_height))
    x2 = max(0, min(float(label[3]), image_width))
    y2 = max(0, min(float(label[4]), image_height))
...
def convert_yolo_to_coco(image_dir: str):
    ...
    print(osp.join(yolo_image_dir + '/*/*/*.jpg'))
    indices = glob.glob(osp.join(yolo_image_dir + '/*/*/*.jpg'))
    total = len(indices)

    # sampling base on id
    indices = [
        x for x in indices if int(osp.basename(x).split('_')[1]) % 600 == 5  # very verbose and ugly but work lol
    ]
    total = len(indices)
    ...
    for idx, image in enumerate(mmengine.track_iter_progress(indices)):
        ...
        img_name = '/'.join(image.split('/')[-3:])[:-4]  # very ugly but should work lol
        label_path = f'{osp.join(yolo_label_dir, img_name)}.json'
        ...
        with open(label_path) as f:
            labels = mmengine.load(f, file_format='json')
            for k, v in labels.items():
                coco_info, obj_count = convert_bbox_info(
                    ' '.join([k, str(v[0]), str(v[1]), str(v[2]), str(v[3])]), idx, obj_count, image_height, image_width)
                if coco_info:
                    dataset['annotations'].append(coco_info)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help='sample rate for train/val/test split, default: 0.1'
    )
